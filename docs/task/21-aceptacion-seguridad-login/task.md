...
- [ ] Construir matriz ADMIN/APODERADO/anónimo con login, refresh, logout, sesiones, bloqueo/desbloqueo, reset/cambio e invitación, positivos y negativos.
- [ ] Probar todos los estados: activo, desactivado, bloqueado, invitado pendiente, enlace expirado/cancelado/consumido y cuenta legacy.
- [ ] Cubrir CSRF, replay, enumeración, permisos, carreras y límites con PostgreSQL real aislado y varios workers; no cerrar solo con mocks.

==================================================
## 3. RECORRIDO E2E
==================================================

- [ ] Ejecutar invitación → correo local → perfil/clave → login → acceso asignado → logout efectivo; capturar evidencia sanitizada.
- [ ] Ejecutar fallos hasta bloqueo → desbloqueo ADMIN → login; luego forgot → mail → reset → rechazo de clave/JWT/refresh anteriores.
- [ ] Probar varias pestañas, sesión expirada durante carga/descarga, pérdida de red y navegador móvil; no duplicar cambios electorales.
- [ ] Revisar teclado, foco, feedback, gestores de claves y links caducados; mantener estilo de INTERFAZ_VISUAL.md.

==================================================
## 4. MIGRACIÓN, SEGURIDAD Y OPERACIÓN
==================================================

- [ ] Verificar upgrade desde BD existente y desde vacía, índices, conservación de cuentas/asignaciones y rollback practicable con backup en base desechable.
- [ ] Documentar despliegue coordinado API/frontend/worker, invalidación de tokens antiguos y expiración absoluta de sesiones.
- [ ] Verificar HTTPS, cookies, CORS, CSRF, secretos/rotación, logs sin credenciales y limpieza de tokens/sesiones/outbox; medir costo de hashing y límites.
- [ ] Documentar monitoreo de fallos, bloqueo abusivo, cola fallida, revocación por incidente, recuperación de ADMIN y retención de auditoría; no registrar más datos personales de los necesarios.
- [ ] Mantener abierta la habilitación externa hasta probar SMTP autorizado, dominio/remitente y URL HTTPS reales. Capturador local no es evidencia de entrega productiva.
- [ ] Registrar MFA/passkeys/SSO como ampliaciones fuera de este alcance; evaluar MFA para ADMIN antes de un despliegue sensible sin declarar certificación NIST/ASVS.

==================================================
## 5. REGRESIÓN ELECTORAL Y DOCUMENTACIÓN
==================================================

- [ ] Ejecutar suites backend/frontend y recorridos electorales críticos: permisos, guardar candidato ausente de padrón, envío y solo lectura, reportes y descargas.
- [ ] Actualizar AUTHENTICATION.md, manuales ADMIN/APODERADO, operación local, README y .env.example con comportamiento implementado.
- [ ] Entregar informe consolidado 16–21 con evidencias locales y externas separadas, pendientes y criterios de habilitación; no marcar completada producción si faltan SMTP/dominio.
