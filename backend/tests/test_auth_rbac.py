...
def test_padron_is_admin_only(client, db_session, users):
    ...
    response = client.post("/api/v1/padron/import", files={"file": ("padron.xlsx", content)}, headers=headers(admin))
    assert response.status_code == 200
    assert db_session.get(AffiliateImportBatch, response.json()["batch_id"]).imported_by == admin.id
...
def test_token_signature_and_missing_user_are_rejected(client, users):
    admin, _ = users
    token = jwt.encode({"sub": str(admin.id), "type": "access", "exp": 4102444800},
                       "wrong-test-signing-key", algorithm=settings.algorithm)
    assert client.get("/api/v1/auth/me", headers={"Authorization": f"Bearer {token}"}).status_code == 401
    missing = create_access_token("99999")
    assert client.get("/api/v1/auth/me", headers={"Authorization": f"Bearer {missing}"}).status_code == 401


def test_database_role_overrides_token_role(client, db_session, users):
    admin, apoderado = users
    forged_role = create_access_token(str(apoderado.id), {"role": "admin"})
    assert client.get("/api/v1/candidates/review", headers={"Authorization": f"Bearer {forged_role}"}).status_code == 403
    previous_headers = headers(admin)
    admin.role = UserRole.APODERADO
    db_session.commit()
    assert client.get("/api/v1/candidates/review", headers=previous_headers).status_code == 403
...
def assigned_scope(db_session, users):
    _, apoderado = users
    election = Election(name="Elección asignada", election_type="interna", election_date=date(2027, 1, 1), active=True)
    office = Office(code="LOCAL", name="Consejo", scope_type="municipal", municipality_based=True,
                    required_positions=2, requires_parity=True, requires_alternation=True, active=True)
    municipalities = [Municipality(name="Asignada"), Municipality(name="Ajena")]
    db_session.add_all([election, office, *municipalities])
    db_session.flush()
    module = UserModule(user_id=apoderado.id, election_id=election.id, office_id=office.id,
                        municipality_id=municipalities[0].id, module_type=UserModuleType.CONSEJOS_LOCALES, enabled=True)
    lists = [ElectoralList(election_id=election.id, office_id=office.id, municipality_id=municipality.id,
                           list_name=municipality.name, created_by=apoderado.id) for municipality in municipalities]
...
def test_list_read_and_validation_respect_assignments(client, db_session, users, assigned_scope):
    admin, apoderado = users
    ...
    assert client.get('/api/v1/validations/list/999?office_type=consejo_local', headers=headers(admin)).status_code == 404
...
def test_unassigned_legacy_candidates_are_not_exposed(client, db_session, users, assigned_scope):
    admin, apoderado = users
    ...
    assert client.post('/api/v1/candidates', json=payload, headers=headers(admin)).status_code == 201
...
@pytest.mark.parametrize('change', ['disabled', 'wrong_election', 'wrong_office', 'other_user'])
def test_assignment_must_match_user_election_office_and_enabled(client, db_session, users, assigned_scope, change):
    admin, apoderado = users
    election, office, municipalities, module, _ = assigned_scope
    if change == 'disabled':
        module.enabled = False
    elif change == 'wrong_election':
        other = Election(name='Otra', election_type='interna', election_date=date(2028, 1, 1))
        db_session.add(other)
        db_session.flush()
        module.election_id = other.id
    elif change == 'wrong_office':
        other = Office(code='OTHER', name='Otro', scope_type='municipal', municipality_based=True, required_positions=2)
        db_session.add(other)
        db_session.flush()
    ...
    payload.update(election_id=election.id, office_id=office.id)
    payload['person']['municipality_id'] = municipalities[0].id
    assert client.post('/api/v1/candidates', json=payload, headers=headers(apoderado)).status_code == 403
    assert client.get('/api/v1/lists/', headers=headers(apoderado)).json() == []
    assert db_session.scalar(select(func.count()).select_from(Person)) == 0
